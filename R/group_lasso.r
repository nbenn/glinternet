group_lasso = function(X, Z, Y, activeSet, betahat, numLevels, lambda, family, tol, maxIter, verbose=FALSE){
  ...
  fit = .Call("R_gl_solver", X, Z, Y, n, betahat[1], betahat[-1], double(n), numLevels, numGroups, indices$cat, indices$cont, indices$catcat, indices$contcont, indices$catcont, lambda, tol, 0.1, maxIter, 0, 0, double(maxIter), ifelse(family=="gaussian", 0, 1), verbose)
  res = fit$res
